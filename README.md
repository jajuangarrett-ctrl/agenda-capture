...
Version 0.6.0 exposes the same reviewed capture modal to FJG Task Manager's
native **Capture Task, Agenda, or Update** launcher. The launcher may prefill the
item text, but the user still chooses a roster member, reviews all fields, and
presses **Save**.
...
FJG Task Manager calls Agenda Capture's native `openCaptureModal(initialText)`
plugin API. This remains review-first and makes no change merely because the
modal opened.
